package com.example.presentation.opengl
...
import com.green.stereovideodemo.*
...
class StereoIotdCameraView(val lifecycleOwner: LifecycleOwner, context: Context, val getDeviceRotation: IGetDeviceRotation): GLSurfaceView(context),
    IStereoVideoView {
    private val videoRenderer by lazy { VideoRenderer(lifecycleOwner, context, getDeviceRotation) }
...
//            }
        ...
        fun updateVideoMatrices() {
            ...
            val rotation = getDeviceRotation.getRotation()
            ...
            //Matrix.rotateM(mMVPMatrixBuffer, 0, rotation * 90f + 90f, 0f, 0f, 1f)
            Matrix.rotateM(mMVPMatrixBuffer, 0, rotation * 90f , 0f, 0f, 1f)
        ...
        override fun onSurfaceCreated(glUnused: GL10?, config: EGLConfig?) {
            ...
            cameraProviderFuture.addListener({
                ...
                val textureSurfaceProvider = Preview.SurfaceProvider { request ->
                    val size = request.resolution
                    videoWidth = size.width.toFloat()
                    videoHeight = size.height.toFloat()
                    updateVideoMatrices()
                    Log.e("Green",
                        "Green textureSurfaceProvider request with size: $videoWidth | $videoHeight"
                    )

                    mSurface = SurfaceTexture(mTextureID)
                    mSurface?.setDefaultBufferSize(videoWidth.toInt(), videoHeight.toInt())
                ...
                val preview = Preview.Builder()
                    .build()
                    .also {
                        it.setSurfaceProvider(textureSurfaceProvider)
                    }
                ...
                try {
                    ...
                    Log.e("StereoCameraView", "Use case binding failed", exc)
                }
